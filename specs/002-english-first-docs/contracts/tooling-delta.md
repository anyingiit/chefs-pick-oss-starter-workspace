...
**修正**：在原有两行之后追加「missing / unexpected」两行，分别列出只出现在英文原文序列中的锚点 id、和只出现在译本序列中的锚点 id，保持原序列中的相对顺序，任一为空时不输出该行。锚点比对逻辑本身不变，仍按有序序列比对。

### 8.7 检查项的诊断消息一律用英文

8.5 与 8.6 新增的四条诊断当初写成了中文，而 `check_template.py` 里另外约 90 条诊断全是英文（`missing required file:`、`unexpected file:`、`{rel}: not valid YAML:` …）。更糟的是 C11 的锚点不一致消息因此变成「两行英文 + 两行中文」拼在同一条消息里——正是本功能要从文档里消除的逐行双语写法，工具却在自己的输出里犯了同一个毛病。

`tools/` 不在宪章「文档语言」条款的管辖范围内（该条只管面向使用者的模板说明文档），所以这不是宪章冲突，而是一致性缺陷；但它出现在一个专门用来禁止双语混排的功能里，留着格外说不过去。

**修正**：四条全部改为英文——C11 的 `missing:` / `unexpected:`，C23 的 `{rel}: {repo} has {value}, contract has {contract_value}` 与 `{repo}: {rel_a} has {value_a}, {rel_b} has {value_b}`。只改措辞，逻辑一律不动。发现于 `/speckit-analyze` 的复查，不是收敛阶段就看出来的。
