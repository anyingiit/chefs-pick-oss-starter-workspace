...
- [X] T045 修复 `tools/check_template.py` 中 C11 锚点序列不一致时的报告措辞，满足 FR-013 最后一句「报告必须指出是哪一份译本缺少或多出哪一个锚点」。当前只打印 `expected:` 与 `found:` 两行完整序列，12 项时需要人工逐项比对。改法：在现有两行之后追加两行，逐字为 `f"    缺少: {missing}"` 与 `f"    多出: {unexpected}"`，其中 `missing` 是出现在英文原文序列、但不在译本序列中的锚点 id 列表，`unexpected` 反之；两个列表都保持原序列中的相对顺序；任一为空时该行不输出。只改这一处报告，不改锚点比对逻辑本身（仍按有序序列比对）。验收：把 `template/.github/chefs-pick/SETUP.zh-CN.md` 中的 `<!-- anchor: placeholders -->` 改成 `<!-- anchor: place-holders -->` 后，`python3 tools/check_template.py --only C11` 的输出中同时出现 `缺少: ['placeholders']` 与 `多出: ['place-holders']`；`git checkout -- template/` 还原后该命令输出 `PASS C11`。
