...
- [ ] T001 在仓库根运行 `python3 -m unittest discover -s tools/tests 2>&1 | tail -3`、`python3 tools/check_template.py | tail -1` 与 `git ls-files template/ | wc -l`，把三个结果写入 `specs/002-english-first-docs/baseline.md`。期望值分别为 `OK`（199 个用例）、`Summary: 22 passed, 0 failed, 0 skipped`、`26`。任一不符则停下来报告，不要开始改造。
