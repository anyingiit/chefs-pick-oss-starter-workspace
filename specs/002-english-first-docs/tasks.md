...
- [X] T006 在 `tools/check_template.py` 中整体重写 `check_c11`，标题改为 `Language structure`，按 tooling-delta §4 的 C11 条目实现全部 10 项断言（纯度、语言入口存在与位置、非译本文档不得有入口、互达、锚点有序序列、锚点数等于标题数、非锚点文档不得有锚点行、译本规范性声明、英文原文不得有声明、首页首行锚点次行标题、译本不得含 `SINGLE_SOURCE_MARKERS`、保留原有 `CLEANUP_COMMAND`/`chefs-pick/SETUP.md`/`chefs-pick/GUIDE.md`/`## Feedback and contact` 断言、`chefs-pick/CHANGELOG.md` 含 `## [`、`chefs-pick/LICENSE` 第 3 行为 `Copyright (c) 2026 Chef's Pick OSS Starter contributors`）。此时对真实模板必然 FAIL，属预期。验收：`python3 tools/check_template.py --only C11 | head -1` 输出 `FAIL C11 Language structure`。
- [X] T007 在 `tools/check_template.py` 中把 `check_c12` 的全部字面量改为英文，按 tooling-delta §4 的 C12 条目：八个字段名、级别取值、六种证据类型、`Data verified:`、`## Excluded candidates`、表头首格 `Field`/`Module`/`Candidate`；删去对 `**入选理由**` 的检查只保留 `**Rationale**`；`**备选方案 / Alternatives**` → `**Alternatives**`；规则为 `2` 时正文须含 `comparable adoption`。日期新鲜度逻辑不动。验收：`python3 tools/check_template.py --only C12 | head -1` 输出 `FAIL C12`（模板尚未改造，属预期）。
- [X] T008 在 `tools/check_template.py` 中调整 C08、C13、C14、C17、C18、C19、C22 七项，按 tooling-delta §4 对应条目：C08 换英文表头；C13/C22 使用新 `CLEANUP_COMMAND`，被删集合含 `.github/README.md`、`.github/README.zh-CN.md` 与 `.github/chefs-pick/` 整个目录；C14 覆盖两个新文件，带 `#` 的链接只校验文件存在不校验锚点；C17 字面量改 `- Delete:`/`- Update:`/`- What you lose:`；C18 新增两项断言（`## Release gates` 一节须有 6 个有序列表项；全文须含 `--update-digests`）；C19 字面量改英文。验收：`python3 tools/check_template.py --only C08,C13,C14,C17,C18,C19,C22 | tail -1` 能正常输出汇总行（不因异常中断）。
- [X] T009 在 `tools/check_template.py` 中新增 `check_c23`（标题 `Contract parity`）与 `check_c24`（标题 `Translation freshness`），并登记进 `CHECKS`，按 tooling-delta §5。C23：比对 `specs/001-chefs-pick-starter/contracts/guidance-layer.md` 与 `template/` 下全部 `.md` 中的 `★ N (owner/repo)`，同一仓库的 `N` 必须相等，不等则报 `owner/repo: contract N1 vs template N2`；只出现在一侧的不报错；契约文件不存在记 SKIP；路径相对仓库根（脚本所在目录的上一级），不受 `--template-dir` 影响。C24：对每份译本读取来源标记，重算英文原文摘要，不等时不带 `--release` 抛 `WarnCheck`、带 `--release` 返回问题列表；缺标记、格式不符、`source` 文件不存在一律 FAIL；无译本记 SKIP。验收：`python3 tools/check_template.py --only C23 | head -1` 输出 `FAIL C23 Contract parity`（12 处漂移尚未修正，属预期）。
- [X] T010 [P] 重写 `tools/tests/test_checks_identity.py` 中的 C11 用例，按 tooling-delta §7：覆盖英文版含中文（FAIL）、中文版含 6 个连续英文词（FAIL）、语言入口缺失（FAIL）、入口位置错（FAIL）、非译本文档出现入口（FAIL）、锚点序列与常量不符（FAIL）、锚点数不等于标题数（FAIL）、译本缺规范性声明（FAIL）、译本含 `| Placeholder | Meaning | Files | Example |`（FAIL）、全部合规（PASS）。必须使用 `tools/tests/helpers.py` 既有的临时模板夹具，不得依赖真实 `template/`。**锚点序列不符的那个用例还必须断言报文内容**：报文须同时含出问题的译本文件名与具体的锚点 id（SC-013 要求"100% 能指名是哪一份译本、哪一个章节锚点"，只断言 FAIL 不够）。验收：`python3 -m unittest tools.tests.test_checks_identity -v 2>&1 | tail -3` 输出 `OK`。
- [X] T011 [P] 更新 `tools/tests/test_checks_selection.py`，把 C12 用例的全部双语字面量改为 T007 采用的英文字面量，并新增反例"规则为 2 但 `**Rationale**` 段落不含 `comparable adoption`"（FAIL）。验收：`python3 -m unittest tools.tests.test_checks_selection 2>&1 | tail -3` 输出 `OK`。
- [X] T012 [P] 更新 `tools/tests/test_checks_links.py`，按 T008 的新清理命令与新文件集合调整 C13、C14、C17、C22 的夹具与断言；新增用例：清理后仍有文件链接到 `.github/README.zh-CN.md`（FAIL）。验收：`python3 -m unittest tools.tests.test_checks_links 2>&1 | tail -3` 输出 `OK`。
- [X] T013 [P] 更新 `tools/tests/test_checks_markers.py`，把 C08 用例的登记表表头改为 `| Placeholder | Meaning | Files | Example |`。验收：`python3 -m unittest tools.tests.test_checks_markers 2>&1 | tail -3` 输出 `OK`。
- [X] T014 [P] 更新 `tools/tests/test_checks_maintaining.py`，把 C18 用例改英文字面量，并新增两个反例：`## Release gates` 只有 5 个有序列表项（FAIL）、全文不含 `--update-digests`（FAIL）。验收：`python3 -m unittest tools.tests.test_checks_maintaining 2>&1 | tail -3` 输出 `OK`。
- [X] T015 [P] 更新 `tools/tests/test_checks_upgrade.py`，把 C19 用例的字面量改为英文。验收：`python3 -m unittest tools.tests.test_checks_upgrade 2>&1 | tail -3` 输出 `OK`。
- [X] T016 [P] 新建 `tools/tests/test_checks_contract_parity.py`，覆盖 C23 的四种情形：两侧数字一致（PASS）、某仓库不一致（FAIL 且报文含 `contract` 与 `template` 两个值）、某仓库只出现在一侧（PASS）、契约文件不存在（SKIP）。用临时目录构造契约文件与模板，不依赖真实文件。验收：`python3 -m unittest tools.tests.test_checks_contract_parity 2>&1 | tail -3` 输出 `OK`。
- [X] T017 [P] 新建 `tools/tests/test_checks_translation.py`，覆盖 C24 与 `--update-digests`：摘要一致（PASS）、摘要过期且不带 `--release`（WARN，退出码 0）、摘要过期且带 `--release`（FAIL，退出码 1）、缺来源标记行（FAIL）、来源标记格式错（FAIL）、`source` 指向的文件不存在（FAIL）、无译本（SKIP）；`--update-digests` 的就地改写正确且再次运行输出 `No digest needed updating.`（幂等）。验收：`python3 -m unittest tools.tests.test_checks_translation 2>&1 | tail -3` 输出 `OK`。
...
- [X] T019 [P] 在 `tools/tests/test_verify_sources.py` 中新增契约写入用例：构造一个含 `★ 1,000 (a/b)` 的临时契约文件与对应模板，运行写入后契约中的值被更新；契约文件不存在时不抛异常。验收：`python3 -m unittest tools.tests.test_verify_sources 2>&1 | tail -3` 输出 `OK`。
...
- [X] T033 [US3] 验证 WARN/FAIL 两条路径确实生效：在 `template/.github/README.md` 的任意一段英文末尾临时追加一个句号，运行 `python3 tools/check_template.py | grep -E '^(WARN|FAIL) C24'` 应出现 `WARN C24` 且 `echo $?` 为 0；再运行 `python3 tools/check_template.py --release | grep -E '^FAIL C24'` 应出现且退出码为 1；随后 `git checkout -- template/.github/README.md` 还原。把三次输出记入 `specs/002-english-first-docs/baseline.md` 的"C24 行为验证"一节。验收：还原后 `git status --short template/` 无输出。
...
Phase 8 的 T043 ~ T048 同样逐条满足这条标准：每条只动一个文件（T043、T045、T047 动 `tools/check_template.py`，T044 动 `tools/tests/test_checks_contract_parity.py`，T046、T048 动 `contracts/tooling-delta.md`），数据结构、消息格式与列表语义都在任务里逐字给出，验收命令自带注入扰动与还原两步，执行者不需要做任何取舍判断。Phase 8 排在 Dependencies 与 Implementation Strategy 之后，是 `/speckit-converge` 只允许追加到文件末尾的结果，不表示它在阶段顺序上晚于这两节；按阶段顺序读时，它紧接 Phase 7。

## Phase 8: Convergence

收敛评估发现两项未完全满足的要求，追加为可执行任务。两项都是「校验够不到」而非「文档写错」，不改 `template/` 下的任何内容。

- [X] T043 修复 `tools/check_template.py` 中 `check_c23()` 的漏检：当前它用 `template_stars[repo] = value` 和 `contract_stars[repo] = value` 两个字典收集数据，同一个仓库的 Star 数在多个文件里出现时只有**最后读到**的那个参与比对，早先文件里的漂移被静默覆盖。实测：把 `template/.github/chefs-pick/SOURCES.md` 里的 `★ 22,527 (renovatebot/renovate)` 改成 `★ 22,999 (renovatebot/renovate)`，C23 仍报 PASS，因为 `UPGRADE-TO-TEAM.md` 排序在后且值正确。36 个仓库中有 12 个跨多个文件，全部可被这样掩盖。改法：把两个字典都换成 `dict[str, list[tuple[str, str]]]`（仓库 → `[(文件相对路径, 数值), ...]`），逐条比对每一处出现；差异信息逐字写成 `f"{rel}: {repo} has {value}, contract has {contract_value}"`；契约内部或 `template/` 内部同一仓库出现两个不同数值时，同样报为问题，逐字写成 `f"{repo}: {rel_a} has {value_a}, {rel_b} has {value_b}"`。不要改 `STAR_RE`，不要改 `tools/verify_sources.py`（它的写入路径已覆盖 README、SOURCES、UPGRADE-TO-TEAM 与契约四处，本身没有问题）。验收：在 `SOURCES.md` 注入上述 `22,999` 后 `python3 tools/check_template.py --only C23` 输出 `FAIL C23` 且退出码为 1，且问题行中出现 `.github/chefs-pick/SOURCES.md`；`git checkout -- template/` 还原后同一命令输出 `PASS C23` 且退出码为 0。
- [X] T044 在 `tools/tests/test_checks_contract_parity.py` 中补三个回归用例，锁住 T043 修复的行为，防止回退：（a）同一仓库的 Star 数出现在两个模板文件中、其中排序靠前的那个与契约不一致时，C23 必须失败；（b）契约内部同一仓库出现两个不同数值时，C23 必须失败；（c）`template/` 内部同一仓库在两个文件中数值不一致时，C23 必须失败。三个用例都用临时目录构造，不得读写仓库中真实的 `specs/001-chefs-pick-starter/contracts/guidance-layer.md` 或 `template/`——本文件已有的 `RealContractUntouchedTests` 守卫必须继续通过。验收：`python3 -m unittest tools.tests.test_checks_contract_parity -v` 全绿，且 `python3 -m unittest discover -s tools/tests` 末尾为 `OK`、用例数不少于 231。
- [X] T045 修复 `tools/check_template.py` 中 C11 锚点序列不一致时的报告措辞，满足 FR-013 最后一句「报告必须指出是哪一份译本缺少或多出哪一个锚点」。当前只打印 `expected:` 与 `found:` 两行完整序列，12 项时需要人工逐项比对。改法：在现有两行之后追加两行，逐字为 `f"    missing:    {missing}"` 与 `f"    unexpected: {unexpected}"`，其中 `missing` 是出现在英文原文序列、但不在译本序列中的锚点 id 列表，`unexpected` 反之；两个列表都保持原序列中的相对顺序；任一为空时该行不输出。只改这一处报告，不改锚点比对逻辑本身（仍按有序序列比对）。同一次改动还要给 `check_c23()` 的返回值加一轮「首次出现保留」去重：逐处比对会在同一个文件里同一条数字重复出现时生成逐字相同的问题行，去重只合并完全相同的行，措辞不变，有任何差异的行都不合并。验收：把 `template/.github/chefs-pick/SETUP.zh-CN.md` 中的 `<!-- anchor: placeholders -->` 改成 `<!-- anchor: place-holders -->` 后，`python3 tools/check_template.py --only C11` 的输出中同时出现 `missing:    ['placeholders']` 与 `unexpected: ['place-holders']`；`git checkout -- template/` 还原后该命令输出 `PASS C11`。
- [X] T046 在 `specs/002-english-first-docs/contracts/tooling-delta.md` 的 §8 末尾追加 8.5 与 8.6 两小节，记录 T043 与 T045 修复的两处缺陷：8.5 写明 C23 原按仓库去重导致跨文件漂移被掩盖（36 个仓库中 12 个受影响），现改为逐处出现比对，并覆盖契约内部与 `template/` 内部的自相矛盾；8.6 写明 C11 锚点不一致时原只打印两条完整序列，现按 FR-013 追加「缺少 / 多出」两行。两小节都要写明是收敛阶段发现的，不是设计时预见的。验收：`grep -c '^### 8\.' specs/002-english-first-docs/contracts/tooling-delta.md` 输出 `6`。
- [X] T047 把 `tools/check_template.py` 中新增的四条诊断消息由中文改为英文，与该文件另外约 90 条诊断保持一致。改前 C11 的锚点不一致消息是「两行英文 + 两行中文」拼在同一条消息里——正是本功能要从文档里消除的逐行双语写法，工具却在自己的输出里犯了同一个毛病。逐字改成：C11 的 `f"\n    missing:    {missing}"` 与 `f"\n    unexpected: {unexpected}"`；C23 的 `f"{rel}: {repo} has {value}, contract has {contract_value}"` 与 `f"{repo}: {rel_a} has {value_a}, {rel_b} has {value_b}"`。只改措辞，不改任何逻辑：逐处比对、自相矛盾检测、排序与去重一律不动。`tools/` 不受宪章「文档语言」条款约束（该条只管面向使用者的模板文档），这条是一致性修复，不是宪章问题。验收：注入锚点改名后 `python3 tools/check_template.py --only C11` 的输出中出现 `missing:` 与 `unexpected:` 且整条消息无中文字符；注入 Star 数漂移后 `--only C23` 的两条问题行全英文；还原后 `python3 tools/check_template.py` 输出 `Summary: 24 passed, 0 failed, 0 warned, 0 skipped`。
- [X] T048 在 `specs/002-english-first-docs/contracts/tooling-delta.md` 上做三处同步：（1）§8 标题由「集成阶段发现并修正的四处遗漏」改为「集成与收敛阶段发现并修正的遗漏」，并在引言末尾注明 8.1 ~ 8.4 来自集成阶段、8.5 起来自收敛阶段——原标题写死「四处」，追加 8.5、8.6 后就与实际条目数不符；（2）§8.6 中引用的消息标签由「缺少 / 多出」改为「missing / unexpected」，与 T047 改后的代码一致；（3）追加 §8.7，记录 T047 修的这条：新增诊断当初写成中文，且 C11 一条消息内中英并排，与全文件英文诊断的惯例冲突，也和本功能自己的主张冲突，现全部改为英文。验收：`grep -c '^### 8\.' specs/002-english-first-docs/contracts/tooling-delta.md` 输出 `7`，且 `grep -c '四处遗漏' specs/002-english-first-docs/contracts/tooling-delta.md` 输出 `0`。
