...
**Scale/Scope**：
- 8 个引导层文件改为纯英文（约 120 KB，其中 12,634 个中文字符要移除）。其中团队升级指引与模板许可证不在宪章点名的六份之列，依 FR-003「禁止同文档并排」一并转换，spec 的 FR-006 已同步写明。
- 2 个新译本文件。
- 4 个文件加章节锚点（约 40 处）。
...
├── baseline.md                    # 第 2 阶段：改造前基线与 C24 行为验证记录（T001、T033 生成）
