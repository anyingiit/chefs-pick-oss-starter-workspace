...
- **语言结构（已由功能 002 取代）**：本节曾规定引导层中每个 Markdown 文件的每一行标题都要中英并列、写法统一为『中文 / English』，这条规则已作废。功能 002 把引导层的语言结构改为：英文是规范版本；中文只以独立的译本文件提供，且只覆盖 `.github/README.md` 与 `.github/chefs-pick/SETUP.md` 这两份文档（对应译本 `README.zh-CN.md`、`SETUP.zh-CN.md`），其余六份文档只有英文版。语言入口行、规范性声明引用块、章节锚点 id、来源标记行这四处标记的逐字规则，见 [language-structure.md](../../002-english-first-docs/contracts/language-structure.md)；八份引导层文档改造后各自的逐文件结构，见 [guidance-layer-en.md](../../002-english-first-docs/contracts/guidance-layer-en.md)。下文 §1 ~ §7 中原先要求标题写成"中文 / English"的地方，已按上述英文规范版本改写；数据表中的 Star 数字与提交哈希未受影响。
...
本节标题为英文规范版本（功能 002 起）；锚点行与语言入口的逐字写法见 [language-structure.md](../../002-english-first-docs/contracts/language-structure.md) §0、§1、§3，中文译本 `README.zh-CN.md` 的对应结构见同一契约 §2、§5 与 [guidance-layer-en.md](../../002-english-first-docs/contracts/guidance-layer-en.md) §2。

1. `# Chef's Pick OSS Starter`（C11 检查第一行标题）
   - 标题下写一段英文简介：各模块都选用社区公认的佼佼者，开箱即用，每项选择都注明来源和认可度。
   - 接着是一段引用块，一句英文："This is the template's setup guide page. Your project's README lives at the repo root's `README.md`; once setup is done and this page is removed, it will show up there instead." 链接写成 `[README.md](../README.md)`。
2. `## What you get`：按必需、推荐、可选三级列出文件，每个文件后附模块编号。
3. `## The picks at a glance`
   - 放一张 16 行的摘要表，用 `<!-- summary:start -->` 和 `<!-- summary:end -->` 包起来。
   - 表头：`| Module | Pick | Adoption | Verified |`。
   - 16 行的逐字内容见 §4.5。
   - 表后附链接 `[Full selection list](chefs-pick/SOURCES.md)`。
4. `## How we pick`：用英文写一遍三级取舍规则（与宪章原则 II 一致），并注明"只看 Star 数不够"。
5. `## Quick start`：
   1. 点击 **Use this template** 创建仓库；
   2. 按 `[Setup checklist](chefs-pick/SETUP.md)` 完成定制；
   3. 查阅 `[Module guide](chefs-pick/GUIDE.md)`。
6. `## Good to know`，写明三点：
...
7. `## Clean up when done`：先用一句英文说明作用，再给出下面这个 `bash` 代码块（C11 检查该命令字符串；命令内容按 [language-structure.md](../../002-english-first-docs/contracts/language-structure.md) §6 改为同时移除中文译本）：
   ...
   之后说明网页操作方法：删除这个文件、`README.zh-CN.md` 和 `.github/chefs-pick` 目录。
8. `## Feedback and contact`（C11 检查这一标题），写明三点：
...
## 2. `.github/chefs-pick/SETUP.md`（起步清单，英文规范版本）

1. `# Setup checklist`，下接一段英文说明：按顺序完成，大约 15 分钟。
2. `## Placeholders`
   - 收录 [markers.md](./markers.md) §1.2 登记表的全部行，表头逐字写作 `| Placeholder | Meaning | Files | Example |`。第 1 列和第 3 列里的占位符、文件路径都放在反引号中，C08 会解析这两列。
   - 给出两条搜索命令：
     - `git grep -n CHANGEME -- . ':(exclude).github/chefs-pick'`：清理前使用，排除本登记表；
     - `git grep -n CHANGEME`：清理后使用。
3. `## Steps`：一张表，列为 `| ID | Kind | Step | How to verify |`，内容必须覆盖下列 9 步，顺序固定。

| ID | 类型 | 步骤要点（用英文写） | 如何确认 |
|---|---|---|---|
| S01 | Required | 替换全部占位符，参见上面的登记表 | 清理前的搜索命令没有输出 |
| S02 | Required | 在仓库首页 About（齿轮图标）中填写简介，可选填写 Topics。如果仓库属于组织账号，还要在 Settings → Moderation options → Reported content 中开启内容举报（个人账号仓库没有这个设置，见 research F10；社区档案接口含 `content_reports_enabled` 字段，页面是否列出这一项在 V4 中确认） | 仓库首页右侧显示简介；社区标准页上与仓库设置相关的项目都已完成 |
| S03 | Required | 开启私有漏洞报告：Settings → Advanced Security → Private vulnerability reporting；附官方文档链接（markers §3） | Security 页出现 **Report a vulnerability** |
| S04 | Required | 开启讨论区：Settings → General → Features → Discussions。如果不开启讨论区，就把两处都改成你自己的求助渠道：`.github/ISSUE_TEMPLATE/config.yml` 中的讨论区链接，以及 `CONTRIBUTING.md` 的 Questions 一节 | New issue 页面上的求助链接可以打开，且 `CONTRIBUTING.md` 指向的渠道与之一致 |
| S05 | Required | 确认许可证：默认 MIT；要更换时，到 choosealicense.com 选好后替换 `LICENSE`，并同步修改 `README.md` 的 License 一节 | 仓库首页显示正确的许可证 |
| S06 | Required | 按项目语言补充三项：`.gitignore` 规则（取自 github/gitignore）；`ci.yml` 的检查与测试步骤，新增的动作要固定到哈希（见 GUIDE 的"Pinning actions"一节）| 推送后 CI 通过 |
| S07 | Optional | 逐个决定推荐和可选模块是否保留，删除方法见 GUIDE | 保留的可选模块都已按说明启用 |
| S08 | Required | 清理起步引导层，运行 README 首页给出的清理命令 | 仓库首页显示你自己的 `README.md` |
| S09 | Required | 最终验收 | Insights → Community Standards 各项都是绿勾；`git grep -n CHANGEME` 没有输出；CI 通过。自检页只会列出你的账号类型支持的项（例如内容举报只对组织账号的公共仓库出现），所以列出的项都应当能补齐 |

## 3. `.github/chefs-pick/GUIDE.md`（模块讲解，只有英文）

1. `# Module guide`，下接一段英文，说明怎样使用本指南。
2. 每个模块一个二级标题，16 个都要有，写法如 `## M01 README`。每个模块下有三个三级标题：
   - `### Why`
   - `### Customize`
   - `### Remove`

   M10 是例外，三级标题改为 `### How to choose`，内容是一张按项目类型推荐工具的表，并写明默认不启用任何工具。

   M01 ~ M03 的"Remove"小节写"Not recommended"，并说明原因。其余模块的"Remove"小节必须写明三点：
   - 要删除哪些文件；
...
3. 其他二级标题，都必须有：
   - `## Adding language-specific rules`：说明如何补充 `.gitignore`、CI 步骤和 Dependabot 生态。
   - `## Pinning actions`：说明以下几点：
     - 查询哈希的命令：`gh api repos/OWNER/ACTION/commits/vX.Y.Z --jq .sha`；
     - 引用写法：`uses: owner/action@<40 位哈希> # vX.Y.Z`；
     - Dependabot 升级哈希时会同时更新版本注释；
     - CI 会拦下没有固定哈希的引用；
     - 附安全使用参考链接（markers §3）。
   - `## Account-level default files`：说明账号级 `.github` 仓库的作用，以及两条规则：自带 Issue 表单会覆盖账号级默认模板；许可证不能由账号统一提供。附官方链接。
   - `## Translating your own README`（原"中文译本 / Chinese translations"一节已由功能 002 整体替换；逐字内容见 [guidance-layer-en.md](../../002-english-first-docs/contracts/guidance-layer-en.md) §6）。
   - `## Tracing the template version`：说明用仓库初始提交的日期对照模板变更记录；也可以在自己的 CHANGELOG 中记一笔所用的模板版本，这一步可选。
   - `## Changing the license`：到 choosealicense.com 选择许可证，替换 `LICENSE`，并修改 `README.md` 的 License 一节。
   - 正文内指向以上小节的锚点链接需同步改成对应的英文锚点（如 `#changing-the-license`），否则 C14 会失败。

## 4. `.github/chefs-pick/SOURCES.md`（选型清单，只有英文；功能 002 起不再要求双语）

### 4.1 结构

1. `# Selection list`
2. 一行数据核实日期，逐字格式：`Data verified: 2026-09-18`（C12 检查；刷新脚本会改写这一行的日期）。
3. `## Selection rules`：写三级规则（与宪章原则 II 一致）、证据类型的六种写法（见 [data-model.md](../data-model.md)，六种写法均取其英文部分：`Exact stars`、`Rounded stars`、`Estimated users`、`Official platform feature`、`De facto standard`、`Not available`），以及复核周期（至少每 6 个月一次，每次发布前也要复核）。
4. `## Modules`：按 M01 ~ M16 的顺序，每个模块一节，格式见 §4.2。
5. `## Excluded candidates`：表头为 `| Candidate | Adoption | Reason |`，行内容取自 research R21，至少 8 行。"Adoption"列统一写成 `★ N (owner/repo)`，已归档的仓库在后面加注"archived"；排除理由只写英文。
6. `## Adoption data`：机器可读的表格，格式见 §4.3。
7. `## About the data`：说明数据的核实方式（GitHub API，Star 数为精确值）、非仓库证据的出处，以及维护说明所在的文件名 `MAINTAINING.md`（写成行内代码，不要写成链接：它与本文件同目录，但由 US4 创建，写成链接会让链接检查在 US2 阶段失败）。
...
| Evidence | De facto standard：…；★ 2,259 (EthicalSource/contributor_covenant) |
| Rule | 1 + 2 |
| Verified | 2026-09-18 |

**Rationale**: (English paragraph, based on the corresponding R section in research)

**Alternatives**

- Name — evidence — reason not picked (English)
```

- 8 个字段标签必须与示例逐字一致（均取英文部分）。
- 级别只能写 `Required`、`Recommended`、`Optional`。M10 写作 `Optional (recommendation only)`。
...
| M09 | Keep a Changelog 1.1.0（https://keepachangelog.com/en/1.1.0/）+ Semantic Versioning 2.0.0（https://semver.org/spec/v2.0.0.html）+ GitHub 自动生成的发布说明（链接见 markers §2.2） | KaC 1.1.0（2.0.0 仍是未发布草稿）；SemVer 2.0.0 | MIT；CC-BY-3.0；官方文档 / Official docs | 精确 Star 数 / Exact stars：★ 6,702 (olivierlacan/keep-a-changelog)、★ 7,853 (semver/semver)；平台官方功能 / Official platform feature：自动生成的发布说明 | 1 | 只用发布说明、不保留 CHANGELOG：平台官方功能 / Official platform feature，会缺少核心要素；Keep a Changelog 2.0.0：事实标准 / De facto standard，尚未发布 |
| M10 | changesets（https://github.com/changesets/changesets）、git-cliff（https://github.com/orhun/git-cliff）、release-please（https://github.com/googleapis/release-please） | 由使用者选定 | MIT；Apache-2.0 或 MIT；Apache-2.0 | 精确 Star 数 / Exact stars：★ 12,408 (changesets/changesets)、★ 12,245 (orhun/git-cliff)、★ 7,509 (googleapis/release-please) | 1 + 2 | GitHub 自动生成的发布说明：平台官方功能 / Official platform feature，即 M09 的默认方案；零依赖，个人项目通常已够用 |
| M11 | actions/starter-workflows 的 Simple workflow（https://github.com/actions/starter-workflows/blob/main/ci/blank.yml）+ actions/checkout（https://github.com/actions/checkout） | actions/checkout@3d3c42e（v7.0.1） | MIT；MIT | 精确 Star 数 / Exact stars：★ 12,082 (actions/starter-workflows)、★ 8,884 (actions/checkout)；固定哈希的依据：平台官方功能 / Official platform feature（GitHub Docs 安全使用参考）与精确 Star 数 / Exact stars ★ 5,694 (ossf/scorecard) | 1 + 2 | super-linter：精确 Star 数 ★ 10,598 (super-linter/super-linter)，镜像庞大、运行慢；markdownlint：精确 Star 数 ★ 6,345 (DavidAnson/markdownlint)，README 含 HTML，容易误报；actionlint：精确 Star 数 ★ 4,235 (rhysd/actionlint)，没有官方动作；github-actions-ensure-sha-pinned-actions：精确 Star 数 ★ 55 (zgosalvez/github-actions-ensure-sha-pinned-actions)，认可度低，且要额外信任一个第三方动作 |
| M12 | GitHub Dependabot（链接见 markers §2.2） | — | MIT（dependabot-core） | 平台官方功能 / Official platform feature；精确 Star 数 / Exact stars：★ 5,773 (dependabot/dependabot-core) | 2 | Renovate：精确 Star 数 ★ 22,527 (renovatebot/renovate)，需要安装 App 并编写配置，许可证为 AGPL-3.0 |
...
## 5. `.github/chefs-pick/MAINTAINING.md`（维护说明，只有英文）

必须包含以下二级标题（均取英文部分）：

- `## Review cadence`：至少每 6 个月一次；每次发布前也要复核，发布时所有数据的核实日期不得早于发布前 30 天。
- `## How to review`，写明四步：
  ...
  3. 检查是否出现重新评估的触发条件。
  4. 更新核实日期。
- `## Re-evaluation triggers`：照抄宪章原则 III 的 3 个触发条件（末次提交超过 12 个月、依赖已被广泛弃用的工具、出现认可度明显更高的候选），写明"末次提交"以默认分支最近一次提交为准、仓库已归档视同满足第 1 条，并说明怎样按三级规则重新取舍。
- `## Action updates`：说明模板仓库上的 Dependabot 合并请求只当作"有新版本"的提示，不要直接在模板仓库合并。正确做法是在开发工作区更新 `template/` 中的哈希和版本注释，以及逐字契约与 research 中记录的固定版本，跑完校验后重新发布；模板仓库的内容更新后，对应的合并请求会自动关闭。直接在模板仓库合并会让下一次发布推送被拒绝，因为发布是单向的。
- `## Recording changes`：选型变更必须在同一次改动中同时更新 `SOURCES.md` 和 `[CHANGELOG.md](CHANGELOG.md)`，用 Added、Changed、Removed 分类，并写明原因。
- `## Release gates`：列出宪章中的门禁，并写明门禁必须在创建 Release **之前**全部通过。自动检查命令为 `python3 tools/check_template.py --release`，写成代码格式；门禁中人工验证的两条要用从模板生成的测试仓库人工验证，步骤见开发工作区中 quickstart 的 C 部分。功能 002 起门禁增至 6 条并新增 `## Keeping the translations in step` 一节，逐字内容见 [guidance-layer-en.md](../../002-english-first-docs/contracts/guidance-layer-en.md) §7。

## 6. `.github/chefs-pick/UPGRADE-TO-TEAM.md`（可选，P4；只有英文）

- **标题**：`# Growing into a team project`。
- **开头说明**：一句英文，说明本页是可选的简要参考，5 分钟内可以读完。
- **`## Common additions`**：一张表，表头为 `| Addition | Purpose | Source | Adoption |`，行内容如下：
  - CODEOWNERS 的团队用法（平台官方功能）；
  - 规则集（平台官方功能，附 markers §3 的链接）；
  - Renovate（★ 22,527 (renovatebot/renovate)）；
  - pre-commit（★ 15,581 (pre-commit/pre-commit)）：个人版已作为可选模块 M14 提供，团队可以在 CI 中强制执行；
  - OpenSSF Scorecard（★ 5,694 (ossf/scorecard)）。
- **`## When you need foundation-level governance`**：列出以下三项：
...
- **`## No longer recommended`**：todogroup/repolinter（★ 465 (todogroup/repolinter)，已归档）。
...
- **首页链接**：本页存在时，`.github/README.md` 的 Quick start 一节末尾要加一行 `(optional) Once your project grows into a team effort, see [Growing into a team project](chefs-pick/UPGRADE-TO-TEAM.md).`。这一行由创建本页的同一个任务添加；不实施 US5 时，首页不得出现这个链接，否则相对链接检查会失败。

## 7. `.github/chefs-pick/CHANGELOG.md`（模板自身的变更记录，逐字；只有英文）
...
发布时，维护者把 `## [Unreleased]` 改为 `## [1.0.0] - YYYY-MM-DD`（Keep a Changelog 的标准写法，只含版本号和日期，按 §0 的例外不受双语规则约束），并在上方新增一个空的 `## [Unreleased]` 节。功能 002 发布后，`## [Unreleased]` 下方新增了一条 `### Changed` 记录，说明引导层改为英文规范版本、中文改以独立译本提供；逐字内容见 [guidance-layer-en.md](../../002-english-first-docs/contracts/guidance-layer-en.md) §10。
