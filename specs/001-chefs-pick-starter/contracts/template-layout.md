...
- **清理命令**：唯一的清理命令是 `git rm -r .github/README.md .github/README.zh-CN.md .github/chefs-pick`，之后提交即可。首页的译本在 `chefs-pick/` 之外，必须显式列出；仍然只有一条命令。在网页上操作时，删除 `.github/README.md`、`.github/README.zh-CN.md` 和 `.github/chefs-pick/` 三者，与上面的命令一一对应；漏掉任何一个都会把另一份首页留在仓库里。
- **清理后必须满足以下条件**（C13 检查）：
